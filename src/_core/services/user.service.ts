...
class UserService extends Vue {
    ...
    public getDisplayUser(tag: string): Promise<AxiosResponse> {
        return axios.get('https://localhost/api/v1/user/' + tag, this._config);
    }

    public follow(tag: string): Promise<AxiosResponse> {
        return axios.patch('https://localhost/api/v1/user/' + tag + '/follow', {}, this._config);
    }

    public unFollow(tag: string): Promise<AxiosResponse> {
        return axios.patch('https://localhost/api/v1/user/' + tag + '/unfollow', {}, this._config);
    }

    public ban(tag: string): Promise<AxiosResponse> {
        return axios.patch('https://localhost/api/v1/user/' + tag + '/ban', {}, this._config);
    }

    public unBan(tag: string): Promise<AxiosResponse> {
        return axios.patch('https://localhost/api/v1/user/' + tag + '/unban', {}, this._config);
    }
